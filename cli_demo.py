...
def main():
    ...
    while not knowledge_name:
        ...
        temp, loaded_files = local_doc_qa.init_knowledge_vector_store(filepath, LANGCHAIN_DEFAULT_KNOWLEDGE_NAME)
        if temp is not None:
            knowledge_name = temp
            ...
            if len(loaded_files) != len(filepath):
                ...
                if reload_flag:
                    knowledge_name = None
                    continue
            print(f"the loaded knowledge_name is 加载的knowledge_name为: {knowledge_name}")
        else:
